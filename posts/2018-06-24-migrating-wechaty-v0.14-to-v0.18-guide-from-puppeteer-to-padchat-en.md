...
date: 2018-06-30
category: project
tags: wechaty, wechaty-puppet-padchat
slug: 2018-06-24-migrating-wechaty-v0.14-to-v0.18-guide-from-puppeteer-to-padchat-en
description: Author: @lijiarui Founder of BotOrange, Author of Wechaty. Wechaty has been updated to version 0.16(BETA) these days and begin to support all kinds of…
---
...
![code](/img/2018/code-migration-en.png)
...
|  |
| --- |
| ``` git pull rm -rf package-lock.json rm -rf node_modules/ npm install ``` |
...
|  |
| --- |
| ``` npm install --save-dev tslint npm install tslint-config-standard ``` |
...
|  |
| --- |
| ``` {     "scripts": {         "lint": "tslint --project . src/**/*.ts src/**/*.tsx",     } } ``` |
...
|  |
| --- |
| ``` wechaty.on('friend', (contact, request) => {   if (!request) {     // this is a friend request confirmation event   } else {     // this is a friend request   } }) ``` |

### After (v0.16 or above)

|  |
| --- |
| ``` wechaty.on('friendship', request => {   switch (request.type()) {     case FriendRequest.Type.RECEIVE:       // this is a friend request request       break     case FriendRequest.Type.CONFIRM:       // this is a friend request confirmation       break   } }) ``` |
...
|  |
| --- |
| ``` - bot.say(new MediaMessage('/image.png') + bot.say(new Message('/image.png') ``` |
...
|  |
| --- |
| ``` const fileBox = FileBox.fromStream( fs.createReadStream(BOT_QR_CODE_IMAGE_FILE), BOT_QR_CODE_IMAGE_FILE, ) ``` |
...
|  |
| --- |
| ``` const isPersonal = contact.personal() const isOfficial = contact.official() ``` |

### After

|  |
| --- |
| ``` /**  * Return the type of the Contact  *  * @returns ContactType - Contact.Type.PERSONAL for personal account, Contact.Type.OFFICIAL for official account  * @example  * const isOfficial = contact.type() === Contact.Type.OFFICIAL  */ const type = Contact.type() ``` |
...
- [BREAKING CHANGE v0.16 room.add return Promise instead of return Promise](https://github.com/Chatie/wechaty/issues/1362)
...
|  |
| --- |
| ``` - const contact = room.member('Huan') + const contact = await room.member('Huan') ``` |
...
|  |
| --- |
| ``` - const mentionList = message.mention() + const mentionList = await message.mention() ``` |
...
|  |
| --- |
| ``` import { Room } from 'wechaty' const room = await Room.create(...) ``` |
...
|  |
| --- |
| ``` - import { Room } from 'wechaty' + import { Wechaty } from 'wechaty'  - const room = await Room.create(...) + const wechaty = new Wechaty() + const room = await wechaty.Room.create(...) ``` |
...
2. NPM [clone-class](https://www.npmjs.com/package/clone-class)
3. <https://github.com/zixia/node-clone-class/issues/5>
...
|  |
| --- |
| ``` const ext = message.ext() // assume the filename is `test.txt` ``` |
