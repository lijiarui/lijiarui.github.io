---
title: Wechaty Puppet Padchat
date: 2018-05-01
category: project
tags: wechaty, wechaty-puppet-padchat
slug: 2018-5-1-wechaty-puppet-padchat
description: WECHATY-PUPPET-PADCHAT 项目地址： https://github.com/lijiarui/wechaty-puppet-padchat 这个模块是是通过WebSocket 连接一个协议服务器来控制iPad 微信，实现个人号的微信接口。
---

# WECHATY-PUPPET-PADCHAT

项目地址：<https://github.com/lijiarui/wechaty-puppet-padchat>
...
![Wechaty Logo](https://raw.githubusercontent.com/Chatie/wechaty/master/docs/images/wechaty-logo-en.png)
...
|  |
| --- |
| ``` npm install wechaty npm install wechaty-puppet-padchat ``` |

## 示例代码

|  |
| --- |
| ``` import { Wechaty } from 'wechaty'  const WECHATY_PUPPET_PADCHAT_TOKEN = 'your-token-here'  const puppet = 'wechaty-puppet-padchat' // 使用ipad 的方式接入。  const puppetOptions = {   token: WECHATY_PUPPET_PADCHAT_TOKEN, }    const bot = new Wechaty({   puppet,   puppetOptions, })  // 设置完成  // 运行 wechaty bot .on('scan', (qrcode, status) => console.log(`Scan QR Code to login: ${status}\nhttps://api.qrserver.com/v1/create-qr-code/?data=${encodeURIComponent(qrcode)}`)) .on('login',            user => console.log(`User ${user} logined`)) .on('message',       message => console.log(`Message: ${message}`)) .start() ``` |
...
Jiarui LI [rui@chatie.io](mailto:rui@chatie.io)
