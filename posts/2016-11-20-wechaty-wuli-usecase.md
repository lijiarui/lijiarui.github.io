---
title: 一个舞娘如何用6行代码写出微信聊天机器人
date: 2016-11-20
category: chatbot
tags: 舞蹈, 舞哩, wechaty, 聊天机器人
slug: 2016-11-20-wechaty-wuli-usecase
description: 作者：@ lijiarui , Entrepreneur, Wechaty Author 我一直和我的小伙伴在业余时间拍摄舞蹈教学视频，叫“舞哩”，原创了几百个视频，近千万的播放，所以就建了微信群来和粉丝交流。
---

作者：@[lijiarui](https://github.com/lijiarui), Entrepreneur, [Wechaty Author](https://github.com/orgs/Chatie/teams/contributor)

我一直和我的小伙伴在业余时间拍摄舞蹈教学视频，叫“舞哩”，原创了几百个视频，近千万的播放，所以就建了微信群来和粉丝交流。

![RuiRui](/img/2016/wechaty-wuli-usecase-ruirui-dance.jpg)
...
![wechaty-pic](/img/2016/wechaty-wuli-usecase-wechaty-pic.jpeg)
...
首次运行以后会有如下的截图：  
![docker-pull](/img/2016/wechaty-wuli-usecase-docker-pull.jpeg)

Docker会自动的将相关的包部署到电脑里面，安装成功后，以后再运行就会是如下的截图了：  
![run-ding](/img/2016/wechaty-wuli-usecase-run-ding.jpeg)
...
|  |
| --- |
| ``` $ npm install --save wechaty $ node mybot.js ``` |
...
|  |
| --- |
| ``` const { Wechaty } = require('wechaty')  Wechaty.instance() .on('scan', (url, code) => console.log(`Scan QR Code to login: ${code}\n${url}`)) .on('login',  user => console.log(`User ${user.name()} logined`)) .on('message',  message => console.log(`Message: ${message.content()}`)) .init() ``` |
...
![qr-code](/img/2016/wechaty-wuli-usecase-qr-code.jpeg)
